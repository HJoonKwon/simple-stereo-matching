...
def test_ssd():
    ...
    assert (
        ...
        and ssd_cost_volume.shape[1] == left_img.shape[1]
        and ssd_cost_volume.shape[2] == left_img.shape[2]
    )
    best_ssd = ssd.get_best(ssd_cost_volume)
    assert (
        best_ssd.shape[0] == left_img.shape[1]
        and best_ssd.shape[1] == left_img.shape[2]
    )
